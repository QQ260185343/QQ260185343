...
namespace AssetBundleBrowser
{
    ...
    internal class AssetBundleBuildTab
    {
        ...
        internal void OnEnable(EditorWindow parent)
        {
            ...
            m_ForceOpenFile = new ToggleData(
               false,
               "完成后打开文件夹",
               "完成后打开文件夹",
               m_UserData.m_OnToggles);
            m_ForceRebuild = new ToggleData(
                false,
                "Clear Folders",
                "如果您选择复制build目录，则将删除build目录的所有内容以及StreamingAssets/AssetBundles",
                m_UserData.m_OnToggles);
            ...
            m_TargetContent = new GUIContent("打包平台", "Choose target platform to build for.");
            m_CompressionContent = new GUIContent("压缩", "Choose no compress不压缩, 标准压缩(LZMA), or chunk based (LZ4)");
        ...
        internal void OnGUI()
        {
            ...
            using (new EditorGUI.DisabledScope(!AssetBundleModel.Model.DataSource.CanSpecifyBuildOutputDirectory))
            {
                ...
                if (GUILayout.Button("打包文件夹", GUILayout.MaxWidth(125f)))
                    OpenDirectory(m_UserData.m_OutputPath);
              ...
              m_ForceOpenFile.state,
              m_ForceOpenFile.content);
                if (newState != m_ForceOpenFile.state)
                {
                    if (newState)
                        m_UserData.m_OnToggles.Add(m_ForceOpenFile.content.text);
                    else
                        m_UserData.m_OnToggles.Remove(m_ForceOpenFile.content.text);
                    m_ForceOpenFile.state = newState;
                }
            ...
            {
                EditorGUILayout.Space();
                m_AdvancedSettings = EditorGUILayout.Foldout(m_AdvancedSettings, "高级设置");
            ...
            // build.
            EditorGUILayout.Space();
            if (GUILayout.Button("Build(加密)"))
            {
                EditorApplication.delayCall += ExecuteBuildEncryption;
            }
            GUILayout.EndVertical();
            EditorGUILayout.EndScrollView();
        }
        private void ExecuteBuild()
        {
            ...
            if (m_ForceOpenFile.state) { OpenDirectory(m_UserData.m_OutputPath); }
          
        }
        private void ExecuteBuildEncryption()
        {
            if (AssetBundleModel.Model.DataSource.CanSpecifyBuildOutputDirectory)
            {
                if (string.IsNullOrEmpty(m_UserData.m_OutputPath))
            ...
            if (m_CopyToStreaming.state)
                DirectoryCopy(m_UserData.m_OutputPath, m_streamingPath);
            Debug.Log(BuildTarget.StandaloneWindows);

            var manifest = BuildPipeline.BuildAssetBundles(m_UserData.m_OutputPath, BuildAssetBundleOptions.ChunkBasedCompression | BuildAssetBundleOptions.ForceRebuildAssetBundle, (BuildTarget)m_UserData.m_BuildTarget);
            foreach (var name in manifest.GetAllAssetBundles())
            {
                var uniqueSalt = Encoding.UTF8.GetBytes(name);
                var data = File.ReadAllBytes(Path.Combine(m_UserData.m_OutputPath, name));
                using (var myStream = new EncryStreamEditor(Path.Combine(m_UserData.m_OutputPath, name), FileMode.Create))
                {
                    myStream.Write(data, 0, data.Length);
                }
                File.WriteAllText(m_UserData.m_OutputPath + "/这是加密的AB包.txt","加密");
            }
            if (m_ForceOpenFile.state) { OpenDirectory(m_UserData.m_OutputPath); }
